...
window.OY_BLOCK_ROSTERTIME = 60;//seconds to keep a node's publci key in the block roster
...
function oy_peer_process(oy_peer_id, oy_data_flag, oy_data_payload) {
    ...
    if (oy_data_flag==="OY_BLOCK_COMMAND") {
        ...
        if (oy_data_payload[3]<oy_time_local+window.OY_MESH_FUTURE&&//check that the broadcast timestamp is not in the future, with buffer leniency
            ...
            oy_block_sync_hop(oy_data_payload[2].slice(), oy_crypt_short, function() {
...
function oy_block_loop() {
    ...
    if (oy_block_time_local!==window.OY_BLOCK_TIME&&(oy_block_time_local/10)%2===0) {
        ...
        setTimeout(function() {
            ...
            for (let oy_key_public in window.OY_BLOCK_ROSTER) {
                if (oy_time_local>window.OY_BLOCK_ROSTER[oy_key_public]) delete window.OY_BLOCK_ROSTER[oy_key_public];
            }
