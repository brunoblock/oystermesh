...
window.OY_MESH_FLOW = 2600000;//characters per second allowed per peer, and for all aggregate non-peer nodes
...
window.OY_BOOST_KEEP = 40;//node IDs to retain in boost memory, higher means more nodes retained but less average node quality
    ...
    if (oy_data_flag==="OY_BLOCK_COMMAND") {
        ...
        if (oy_data_payload[3]===window.OY_SELF_SHORT) {
            if (oy_data_payload[4]===window.OY_BLOCK_SYNC_DYNAMIC&&oy_time_local-window.OY_BLOCK_TIME>=window.OY_BLOCK_SECTORS[0][0]&&oy_time_local-window.OY_BLOCK_SECTORS[0][0]+window.OY_BLOCK_SECTORS[1][0]) {
                oy_key_sign(window.OY_SELF_PRIVATE, oy_data_payload[5]+window.OY_BLOCK_SYNC_HASH, function(oy_challenge_crypt) {
        ...
        if (oy_data_payload.length!==8||typeof(oy_data_payload[0])!=="object"||oy_data_payload[1].length===0||!oy_channel_check(oy_data_payload[2])) {
                                    ...
                                    if (oy_data_payload[3][i][1][8][x][1]!==oy_data_payload[3][i][1][5]&&oy_channel_approved(oy_data_payload[2], oy_data_payload[3][i][1][8][x][1])) {
                                        oy_key_verify(oy_data_payload[3][i][1][8][x][1], oy_data_payload[3][i][1][8][x][0], oy_data_payload[3][i][1][4], function(oy_key_valid) {
                                            if (oy_key_valid===true) {
                                                ...
                                                else if (typeof(window.OY_CHANNEL_KEEP[oy_data_payload[2]][oy_data_payload[3][i][0]][8])!=="undefined") {
                                                    let oy_push = true;
                                                    for (let y in window.OY_CHANNEL_KEEP[oy_data_payload[2]][oy_data_payload[3][i][0]][8]) {
                                                        if (window.OY_CHANNEL_KEEP[oy_data_payload[2]][oy_data_payload[3][i][0]][8][y][1]===oy_data_payload[3][i][1][8][x][1]) {
                                                            oy_push = false;
                                                            break;
                                                        }
                                                    }
                                                    if (oy_push===true) window.OY_CHANNEL_KEEP[oy_data_payload[2]][oy_data_payload[3][i][0]][8].push([oy_data_payload[3][i][1][8][x][0], oy_data_payload[3][i][1][8][x][1]]);
                                                }
            ...
            if (window.OY_PEER_COUNT>=window.OY_BLOCK_PEERS_MIN) {
                setTimeout(function() {
                    ...
                    oy_key_sign(window.OY_SELF_PRIVATE, oy_sync_time+window.OY_BLOCK_SYNC_HASH+oy_dive_reward, function(oy_sync_crypt) {
                        oy_data_route("OY_LOGIC_ALL", "OY_BLOCK_SYNC", [[], window.OY_BLOCK_SYNC_DYNAMIC, oy_sync_time, oy_sync_crypt, oy_sync_command, window.OY_SELF_PUBLIC, oy_dive_reward]);
