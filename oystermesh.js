...
window.OY_MESH_FLOW = 168000;//characters per second allowed per peer, and for all aggregate non-peer nodes
...
window.OY_BLOCK_DENSITY = 0.65;//higher means block OY_LOGIC_ALL packets are more spread out
...
window.OY_NODE_BLACKTIME = 180;//seconds to blacklist a punished node for
...
window.OY_CHANNEL_KEEPTIME = 5;//channel bearing nodes are expected to broadcast a logic_all packet within this interval
...
window.OY_CHANNEL_RESPOND_MAX = 10;//max amount of broadcast payloads to send in response to a channel recover request, divided by online count
...
function oy_peer_process(oy_peer_id, oy_data_flag, oy_data_payload) {
    ...
    if (oy_data_flag==="OY_BLOCK_COMMAND") {
        ...
        oy_node_initiate(oy_data_payload);
        return true;
    }
}
